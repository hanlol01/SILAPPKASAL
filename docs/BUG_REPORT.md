...
| Bug ID | Severity | Status | Affected Page | Expected Behavior | Actual Behavior | Possible Root Cause | Recommended Fix |
|---|---|---|---|---|---|---|---|
| UX09B-BUG-001 | Low | Verified | `frontend/src/routes/dashboard.cases.$id.tsx` | The route file should have a single consolidated `import { ... } from "lucide-react";` block, matching the project's convention of one import per module per source. | Verified fixed in UX-09B hotfix QA recheck: `Inbox` is now included in the existing `lucide-react` import block, and the separate `import { Inbox } from "lucide-react";` line has been removed. TypeScript, build, and lint all pass after the hygiene fix. | Previous root cause was that the UX-09B edit added the `Inbox` icon next to the new `EmptyState` import instead of merging it into the existing `lucide-react` icon block. | No further fix required. Keep consolidated imports as the convention for future route edits. |
