...
export function ReporterArticleDetailPage({ slug, section, area = "portal" }: { slug: string; section: "education" | "policy"; area?: "portal" | "dashboard" }) {
  ...
  return <article className="mx-auto w-full max-w-7xl space-y-8">
    {area === "portal" ? <ReporterInformationBreadcrumb current={article.title} section={{ label: sectionTitle, to: listTo as "/portal/information-center/education" | "/portal/information-center/policies" }} /> : <DashboardInformationBreadcrumb current={article.title} section={{ label: sectionTitle, to: listTo as "/dashboard/information-center/education" | "/dashboard/information-center/policies" }} />}
    ...
    <div className="mx-auto max-w-3xl"><ContentDocumentPreview document={article.body ?? null} media={article.inline_images ?? []} /></div>
    ...
    {article.related_articles?.length ? <section className="space-y-5"><h2 className="text-2xl font-semibold">{t("article.related")}</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{article.related_articles.map((related) => <PublishedArticleCard key={related.public_id} article={related} area={area} />)}</div></section> : null}
