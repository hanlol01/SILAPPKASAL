...
export const Route = createFileRoute("/portal/information-center/faq")({ component: ReporterFaqPage });
function ReporterFaqPage() {
  ...
  return <div className="mx-auto w-full max-w-5xl space-y-7"><ReporterInformationBreadcrumb current={t("sections.faq.title")} />
    <header className="rounded-3xl border bg-gradient-to-br from-primary/10 via-background to-accent/10 p-6 sm:p-8"><p className="text-sm font-medium text-primary">{t("eyebrow")}</p><h1 className="mt-2 text-3xl font-semibold">{t("sections.faq.title")}</h1><p className="mt-3 text-muted-foreground">{t("views.faq.description")}</p></header>
    <form onSubmit={submit} role="search" className="flex gap-2 rounded-2xl border bg-card p-4"><div className="relative flex-1"><Label htmlFor="faq-search" className="sr-only">{t("filters.searchLabel")}</Label><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="faq-search" className="h-11 pl-9" value={input} maxLength={100} placeholder={t("filters.faqSearchPlaceholder")} onChange={(e) => setInput(e.target.value)} /></div><Button type="submit" className="min-h-11">{t("filters.searchAction")}</Button></form>
