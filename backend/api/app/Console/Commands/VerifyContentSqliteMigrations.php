...
final class VerifyContentSqliteMigrations extends Command
{
    protected $signature = 'content:verify-sqlite-migrations';

    protected $description = 'Verify C1 migrations and repair rollback/re-apply on guarded SQLite memory';

    public function handle(TestDatabaseGuard $guard): int
    {
        $target = $guard->assertSafe();
        if ($target['driver'] !== 'sqlite' || $target['database'] !== ':memory:') {
            throw new RuntimeException('This command runs only against guarded SQLite :memory:.');
        }
        ...
        $this->info('SQLite migrate:fresh, seed, repair rollback, and repair re-apply passed.');
